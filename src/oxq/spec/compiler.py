...
def _build_strategy_py_artifact(
    ...
    lines = [
        ...
        "",
        "This file shows the strategy in regular SDK code form: define the",
        "strategy, define the universe, prepare data, configure the broker, run",
        "the backtest, and print metrics.",
        "",
        "Read the functions below from top to bottom:",
        "1. define_strategy()",
        "2. define_universe()",
        "3. prepare_data()",
        "4. define_indicators() / define_signals() / define_portfolio() / define_rules()",
        "5. define_broker()",
        "6. run_backtest()",
        "7. print_metrics()",
        "8. main()",
        ...
        "from oxq.trade.slippage import PercentageSlippage",
        "",
        "def load_strategy_spec() -> StrategySpec:",
        '    """Load the canonical strategy specification next to this file."""',
        '    return StrategySpec.from_yaml(Path(__file__).parent / "strategy_spec.yaml")',
        "",
        "",
        "def load_compiled_plan() -> dict[str, Any]:",
        '    """Load the compiled runtime plan next to this file."""',
        '    path = Path(__file__).parent / "compiled_plan.json"',
        ...
        "    stored_hash = compiled_plan.get('spec_hash')",
        ...
        '    """Define the strategy from strategy_spec.yaml."""',
        "    # The strategy includes indicators, signals, portfolio optimizer,",
        "    # and rules. The source of truth remains the adjacent YAML file,",
        "    # so this generated script stays readable instead of duplicating",
        "    # every field as Python constants.",
        "    return load_strategy_spec()",
        "",
        "",
        "def strategy_overview() -> dict[str, Any]:",
        '    """Return high-level strategy metadata for review."""',
        "    return deepcopy(load_compiled_plan().get('strategy', {}))",
        "",
        "",
        "def define_universe(spec: StrategySpec):",
        '    """Define the tradable universe used by the backtest engine."""',
        "    return compile_universe(spec)",
        "",
        "",
        "def describe_universe() -> dict[str, Any]:",
        '    """Return the universe configuration in review-friendly form."""',
        ...
        '    """Prepare the local market data provider used by Engine.run."""',
        "    plan = load_compiled_plan()",
        "    data = plan.get('data', {})",
        "    market = plan.get('market', {})",
        "    data_dir = data.get('effective_data_dir') or data.get('data_dir')",
        "    return LocalMarketDataProvider(",
        "        data_dir=Path(data_dir) if data_dir else None,",
        "        currency=spec.market.currency,",
        "        calendar=market.get('runtime_calendar') or spec.market.calendar,",
        "    )",
        "",
        "",
        "def describe_data() -> dict[str, Any]:",
        '    """Return data loading assumptions used by the run."""',
        "    return deepcopy(load_compiled_plan().get('data', {}))",
        "",
        "",
        "def define_indicators() -> dict:",
        '    """Define indicators used by the strategy."""',
        ...
        "    signals = load_compiled_plan().get('signals', {})",
        "    return deepcopy(signals.get('indicators', {}))",
        "",
        "",
        "def define_signals() -> dict:",
        '    """Define signal rules consumed by the portfolio layer."""',
        ...
        "    signals = deepcopy(load_compiled_plan().get('signals', {}))",
        "    return {",
        "        'signal_time': signals.get('signal_time'),",
        "        'rules': signals.get('rules', {}),",
        "        'terminal_signals': signals.get('terminal_signals', []),",
        "        'event_signal_types': signals.get('event_signal_types', []),",
        "    }",
        "",
        "",
        "def define_portfolio() -> dict:",
        '    """Define how indicators or signals become target weights."""',
        ...
        "    return {",
        "        'spec': deepcopy(define_strategy().to_dict().get('portfolio', {})),",
        "        'compiled': deepcopy(load_compiled_plan().get('portfolio', {})),",
        "    }",
        "",
        "",
        "def define_rules() -> list[dict]:",
        '    """Define runtime rules applied around portfolio construction."""',
        "    # Rules constrain when or how trades happen. A rebalance rule, for",
        "    # example, must be visible here before the run can claim it was",
        "    # executed.",
        "    return deepcopy(load_compiled_plan().get('runtime_rules', []))",
        "",
        "",
        ...
        "    spec_dict = define_strategy().to_dict()",
        "    plan = load_compiled_plan()",
        "    return {",
        "        'execution': deepcopy(spec_dict.get('execution', {})),",
        "        'compiled_execution': deepcopy(plan.get('execution', {})),",
        "        'cost': deepcopy(spec_dict.get('cost', {})),",
        "        'benchmark': deepcopy(spec_dict.get('benchmark', {})),",
        "        'validation': deepcopy(spec_dict.get('validation', {})),",
        "        'metrics': deepcopy(spec_dict.get('metrics', {})),",
        "    }",
        "",
        "",
        ...
        "",
        "",
        "def describe_broker() -> dict[str, Any]:",
        '    """Return broker settings without instantiating the broker."""',
        "    plan = load_compiled_plan()",
        "    execution = plan.get('execution', {})",
        "    cost = plan.get('cost', {})",
        "    return {",
        ...
        "",
        "",
        "def resolve_backtest_period(spec: StrategySpec) -> tuple[str, str]:",
        '    """Resolve the train/test date range used by the run."""',
        "    validation = load_compiled_plan().get('validation', {})",
        "    train = list(validation.get('train_period') or spec.validation.train_period)",
        "    test = list(validation.get('test_period') or spec.validation.test_period)",
        "    start = train[0] if train else (test[0] if test else '2018-01-01')",
        "    end = test[1] if test else (train[1] if train else '2025-12-31')",
        "    return start, end",
        "",
        ...
        "        start=start,",
        "        end=end,",
        "        initial_cash=execution.get('initial_cash', spec.execution.initial_cash),",
        "        universe=universe,",
        ...
        "",
        "",
        "def print_metrics(result: RunResult) -> None:",
        '    """Print headline performance metrics from a completed run."""',
        ...
        "    return [",
        "        {'phase': 'strategy', 'function': 'define_strategy'},",
        "        {'phase': 'universe', 'function': 'define_universe'},",
        "        {'phase': 'data', 'function': 'prepare_data'},",
        "        {'phase': 'indicator', 'function': 'define_indicators'},",
        "        {'phase': 'signal', 'function': 'define_signals'},",
        "        {'phase': 'portfolio', 'function': 'define_portfolio'},",
        "        {'phase': 'rule', 'function': 'define_rules'},",
        "        {'phase': 'broker', 'function': 'define_broker'},",
        "        {'phase': 'trade_simulation', 'function': 'run_backtest'},",
        "        {'phase': 'metrics', 'function': 'print_metrics'},",
        "    ]",
        "",
        "",
        "def describe() -> dict:",
        '    """Return review material assembled by the generated functions."""',
        "    return {",
        '        "status": "ready",',
        '        "strategy_flow": simulate_trading_flow(),',
        '        "strategy": strategy_overview(),',
        '        "universe": describe_universe(),',
        '        "data": describe_data(),',
        ...
        '        "metrics": deepcopy(load_compiled_plan().get("metrics", {})),',
        "    }",
        "",
        "",
        "def build_strategy(spec: StrategySpec | None = None):",
        '    """Build the runtime Strategy object for compatibility checks.',
        "",
        "    Workspace-local component manifests are reloaded when this file is",
        "    imported from a completed run directory.",
        '    """',
        ...
        "        source_spec = spec if spec is not None else define_strategy()",
        "        return compile_strategy(source_spec)",
        "",
        "",
        "def main(dry_run: bool = False):",
        '    """Run the strategy script or return the structured plan in dry-run mode."""',
