...
Demonstrates spec validation rules by creating 5 specs and validating them.
...
spec.cost.fee_rate = 0.001; spec.cost.slippage_rate = 0.001
...
# ── 2. Valid momentum Top-N ────────────────────────────────────────────
spec = StrategySpec.template(strategy_id="momentum_topn_valid", hypothesis="Relative strength has short-term continuation.")
spec.name = "Momentum Top-N (Valid)"
spec.universe.symbols = ["SPY", "QQQ", "IWM"]
spec.signal.indicators = {
    "momentum_20": IndicatorDef(type="NdayReturn", params={"column": "close", "period": 20}),
}
spec.signal.rules = {"positive_mom": SignalRuleDef(type="Threshold", params={"column": "momentum_20", "threshold": 0, "relationship": "gt"})}
...
spec.validation.test_period = ["2022-01-01", "2025-12-31"]
spec.validation.required_oos = True
_print_result("2. momentum_topn_valid (should PASS with survivorship warning)", validate(spec))
...
# ── 4. Zero cost model (invalid) ───────────────────────────────────────
spec = StrategySpec.template(strategy_id="zero_cost_invalid", hypothesis="Deliberately invalid: zero-cost model")
spec.universe.symbols = ["SPY"]
spec.signal.signal_time = "close_t"
spec.execution.trade_time = "next_open"
spec.execution.fill_price_mode = "next_open"
spec.cost.fee_rate = 0.0; spec.cost.slippage_rate = 0.0
spec.benchmark.symbols = ["SPY"]
spec.validation.test_period = ["2022-01-01", "2025-12-31"]
_print_result("4. zero_cost_invalid (should FAIL)", validate(spec))
...
print(f"\n{'='*50}")
print("  Done — 5 specs validated (2 pass, 2 fail, 1 warn)")
