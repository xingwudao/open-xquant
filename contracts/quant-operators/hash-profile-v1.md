...
Schema 和语义 validator 的 manifest 对象按 JSON 类型递归相同，布尔值与数值
不得因宿主语言相等规则而等同。每次 binding 验证 `MUST` 只读取路径一次，
并用同一份内存 byte 快照完成严格解码、对象比较和 manifest digest 计算。
实现 `MUST NOT` 分别验证一个内存对象并散列另一个文件或文件快照。
...
- `validate_operator_binding(binding, manifest, manifest_path, source_root,
  ...
  `manifest_path` 的单次读取快照，按 JSON 类型确认结果与 `manifest` 对象相同，
  并用该快照计算 manifest digest；再验证 manifest 语义，并以真实路径复算
  source tree、正式 implementation artifact 和四项 contract surface 摘要，
  拒绝所有重复 identity/provenance 字段不一致。
