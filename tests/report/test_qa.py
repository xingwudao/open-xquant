...
def test_report_qa_accepts_explicit_cjk_font_file_registration(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_accepts_registered_cjk_font_name_variable(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_addfont_before_rcparams_font_property_name(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_addfont_before_rcparams_assignment(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_accepts_direct_cjk_font_property_on_text(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_accepts_annotated_cjk_font_property_on_text(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            "import matplotlib.font_manager as fm\n"
            "import matplotlib.pyplot as plt\n"
            "fp: fm.FontProperties = fm.FontProperties(fname='/System/Library/Fonts/STHeiti Light.ttc')\n"
            "plt.title('策略净值', fontproperties=fp)\n"
        ),
        encoding="utf-8",
    )
...
def test_report_qa_requires_cjk_font_property_before_text_call(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_cjk_font_property_on_cjk_text_call(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_accepts_cjk_plot_label_with_fonted_legend(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_fonted_legend_for_same_cjk_label_receiver(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            ...
            "ax.plot([1, 2], [3, 4], label='策略净值')\n"
            "ax.legend()\n"
            "ax2.plot([1, 2], [4, 3], label='benchmark')\n"
            "ax2.legend(prop=fp)\n"
        ),
        encoding="utf-8",
    )
...
def test_report_qa_warns_for_cjk_text_in_helper_using_outer_font_property(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_rcparams_before_helper_call(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_warns_for_cjk_text_in_helper_using_outer_rcparams(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_warns_for_rcparams_inside_helper_before_cjk_text(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_rcparams_inside_helper_before_cjk_text(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_warns_for_nested_helper_call_after_rcparams(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_accepts_cjk_text_fontdict_font_property(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            ...
            "import matplotlib.pyplot as plt\n"
            "fp = fm.FontProperties(fname='/System/Library/Fonts/STHeiti Light.ttc')\n"
            "plt.title('策略净值', fontdict={'fontproperties': fp})\n"
        ),
        encoding="utf-8",
    )
    figure = tmp_path / "equity.png"
...
def test_report_qa_accepts_positional_cjk_legend_labels_with_font(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_selected_cjk_font_file_to_be_registered(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_rejects_font_property_object_in_rcparams(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_rejects_reassigned_cjk_font_path_variable(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_rejects_reassigned_cjk_font_property_variable(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            ...
            "import matplotlib.pyplot as plt\n"
            "fp = fm.FontProperties(fname='/System/Library/Fonts/STHeiti Light.ttc')\n"
            "fp = fm.FontProperties(fname='/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf')\n"
            "plt.title('策略净值', fontproperties=fp)\n"
        ),
        encoding="utf-8",
    )
...
def test_report_qa_rejects_reassigned_registered_cjk_font_name(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_does_not_treat_method_call_as_helper_invocation(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_requires_every_cjk_legend_label_to_be_fonted(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_keeps_font_property_variables_in_python_scope(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_accepts_cjk_label_variables_with_font_property(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            ...
            "label = '策略净值'\n"
            "fp = fm.FontProperties(fname='/System/Library/Fonts/STHeiti Light.ttc')\n"
            "plt.title(label, fontproperties=fp)\n"
        ),
        encoding="utf-8",
    )
    figure = tmp_path / "equity.png"
...
def test_report_qa_accepts_stsong_cjk_font_file_marker(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            "import matplotlib.font_manager as fm\n"
            "import matplotlib.pyplot as plt\n"
            "fp = fm.FontProperties(fname='/System/Library/Fonts/STSong.ttf')\n"
            "plt.title('策略净值', fontproperties=fp)\n"
        ),
        encoding="utf-8",
    )
...
def test_report_qa_matches_registered_font_path_variable_to_literal_property(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_warns_for_registered_font_path_inside_helper_scope(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_warns_for_returned_axes_cjk_label_with_fonted_legend(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            ...
            "fp = fm.FontProperties(fname='/System/Library/Fonts/STHeiti Light.ttc')\n"
            "ax = sns.lineplot(x=[1, 2], y=[1, 3], label='策略净值')\n"
            "ax.legend(prop=fp)\n"
        ),
        encoding="utf-8",
    )
...
def test_report_qa_ignores_registered_font_helper_in_conditional_branch(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
...
def test_report_qa_rejects_unrelated_addfont_method_for_registration(tmp_path) -> None:
    run_dir = _write_qa_run(tmp_path)
    script = run_dir / "report_assets/scripts/plot.py"
    script.parent.mkdir(parents=True)
    script.write_text(
        (
            ...
            "\n"
            "cache = Cache()\n"
            "cache.addfont('/System/Library/Fonts/STHeiti Light.ttc')\n"
            "fp = fm.FontProperties(fname='/System/Library/Fonts/STHeiti Light.ttc')\n"
            "plt.rcParams['font.family'] = fp.get_name()\n"
            "plt.title('策略净值')\n"
        ),
